...
const deleteCourse= async()=>{
  ...
  if(!confirm) return;
  const req=await fetch(`/api/courses/${id}`,{
    method:'DELETE'
  })
  toast.success("course delete sucessful")
  navigate("/courses")
}
    ...
    <>
      ...
      {userType=="admin"&&(
        ...
        <Link to={`/edit-course/${id}`} className="flex bg-blue-500 hover:bg-blue-600 text-white font-bold  rounded-full h-10 w-32 focus:outline-none focus:shadow-outline justify-center items-center">
          ...
          <button onClick={deleteCourse} className="flex bg-red-500 hover:bg-red-600 text-white font-bold  rounded-full h-10 w-32 focus:outline-none focus:shadow-outline  justify-center items-center">
            Remove Course
          </button>
          </>
